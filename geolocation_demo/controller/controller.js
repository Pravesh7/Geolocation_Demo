var { location_Model } = require('../modals/modal');
...
module.exports.getLocation = async (req, res) => {
    ...
    try {
        const result = await location_Model.find({
            location: { $near: { $geometry: { type: "Point", coordinates: [Number(req.query.long) , Number(req.query.lat)] }, $minDistance: 0, $maxDistance: 1000} }
